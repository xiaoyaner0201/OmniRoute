...
export function registerPersistenceBackendConformance(cases: BackendConformanceCase[]): void {
  for (const backendCase of cases) {
    ...
    test(`[${backendCase.name}] migration locks are FIFO and release is idempotent`, async () => {
      const backend = await backendCase.create();
      try {
        await backend.initialize();
        ...
        const secondPromise = backend.migrationLocks
          ...
          .then((lock) => {
            acquisitionOrder.push(lock.ownerId);
            return lock;
          });
        const thirdPromise = backend.migrationLocks
          .acquire({ ownerId: "third", timeoutMs: 100 })
          .then((lock) => {
            acquisitionOrder.push(lock.ownerId);
            return lock;
          });
    ...
    test(`[${backendCase.name}] close drains queued migration locks before owner release`, async () => {
      const backend = await backendCase.create();
      await backend.initialize();
      const first = await backend.migrationLocks.acquire({ ownerId: "first", timeoutMs: 100 });
